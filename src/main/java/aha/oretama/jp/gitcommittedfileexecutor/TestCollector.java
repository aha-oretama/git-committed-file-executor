...
class TestCollector extends InvisibleAction implements Serializable {
    ...
    private transient GitCommittedFileExecutor testExecutor;
    ...
    public TestCollector(AbstractBuild<?, ?> collector, GitCommittedFileExecutor testExecutor, int ordinal) {
    ...
    public void collect(AbstractBuild<?,?> build, TaskListener listener) {
        if (collector == null) {
            return; // must be deserialized. pretend as if this action doesn't exist.
        }

        try {
            listener.getLogger().println("Collecting test reports for the master build: "+ ModelHyperlinkNote
                .encodeTo(collector));

            final FilePath src = build.getWorkspace();
            if (src == null) {
                return; // trying to be defensive in case of catastrophic build failure
            }
